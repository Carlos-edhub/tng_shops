...
lines = []
lines.append("╔══════════════════════════════════════════════════════════════════════════════════╗")
lines.append("║                      CATÁLOGO TNG SHOPS - 46 PERFUMES                          ║")
lines.append("╚══════════════════════════════════════════════════════════════════════════════════╝")
lines.append("")

for p in products:
    pid, name, brand, cat, notes, price, vol = p
    fname = f"{brand.lower()}_{name.lower().replace(' ', '_').replace(\"'\", '_').replace('é', 'e').replace('è', 'e').replace('í', 'i')}.png"
    lines.append(f"ID {pid:>2} | {brand:>7} | {cat:>6} | {price:>5}€ | {vol:>5}")
    lines.append(f"       {name}")
    lines.append(f"       Notas: {notes}")
    lines.append(f"       Imagen → {fname}")
    lines.append(f"       Búscala en: Fragrancenet, Fragrantica, Notino, Amazon")
    lines.append("")

lines.append("══════════════════════════════════════════════════════════════════════════════════")
lines.append("Instrucciones:")
lines.append("1. Busca cada perfume en Fragrancenet / Fragrantica / Notino / Amazon")
lines.append("2. Descarga la imagen del producto (la oficial, sin watermark grande)")
lines.append("3. Renómbrala exactamente como dice 'Imagen →' arriba")
lines.append("4. Ponla en la carpeta public/images/productos/")
lines.append("5. Dame las imágenes y las reemplazo automáticamente")
lines.append("══════════════════════════════════════════════════════════════════════════════════")

with open(r"D:\TNGSHOPS\PaginaWeb\CATALOGO_TNG.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(lines))
